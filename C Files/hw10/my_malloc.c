...
static metadata_t *split_block(metadata_t *block, size_t size) {
    ...
    uintptr_t new_block_addr = (uintptr_t) block + sizeof(metadata_t) - total_size + block->size;
    block->size -= total_size;
    metadata_t *new_block = (metadata_t *) new_block_addr;
    new_block->size = size;
    return (metadata_t *) new_block_addr;
}
...
static metadata_t *find_best_fit(size_t size) {
    ...
    while (curr) {
        if (curr->size == size) {
            return curr;
        } else if (curr->size > size && (!best_fit || curr->size < best_fit->size)) {
            best_fit = curr;
        }
        curr = curr->next;
    }
...
void *my_malloc(size_t size) {
    ...
    if (best_fit && size == best_fit->size) {
        remove_from_addr_list(best_fit);
        return (metadata_t *) ((char *) best_fit + sizeof(metadata_t));
    } else if (best_fit && best_fit->size > size && best_fit->size - size < MIN_BLOCK_SIZE) {
        remove_from_addr_list(best_fit);
        return (metadata_t *) ((char *) best_fit + sizeof(metadata_t));
    } else if (best_fit && best_fit->size > size) {
        metadata_t *new_block = split_block(best_fit, size);
        ...
        new_block->next = NULL;
        my_free((metadata_t *) ((char *) new_block + sizeof(metadata_t)));
