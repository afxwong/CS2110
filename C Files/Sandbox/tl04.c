...
int set_character(struct ascii_image *image, int row, int col, char c)
{
    ...
    if (!image || !image->data) {
        return FAILURE;
    } else if (row < 0 || row >= image->height) {
        return FAILURE;
    } else if (col < 0 || col >= image->width) {
        return FAILURE;
    } else if (!is_printable(c)) {
        return FAILURE;
    } else {
        int offset = col + (row * image->width);
        image->data[offset] = c;
        return SUCCESS;
    }
...
int replace_extension(struct ascii_image *image, char *extension)
{
    if (!image || !image->data) {
        ...
        int oldLen = strlen(image->name);
        int newLen = oldLen + strlen(extension) + 1;
        ...
        if (lastDot == -1) {
            ...
            for (int i = lastDot; i < newLen - 1; i++) {
                newName[i] = extension[i - (lastDot)];
            }
