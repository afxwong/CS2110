...
int set_character(struct ascii_image *image, int row, int col, char c)
{
    ...
    if (image == NULL || image->data == NULL || row < 0
        || row >= image->height || col < 0 || col >= image->width || !is_printable(c))
        return FAILURE;
...
{
    // TODO: IMPLEMENT THIS FUNCTION! SEE .pdf FOR DETAILS
    if (image == NULL || image->data == NULL || row1 < 0
        || row2 < 0 || col1 < 0 || col2 < 0 || !is_printable(c))
        return FAILURE;

    int rowdiff = row2 - row1;
    int coldiff = col2 - col1;
    if (rowdiff != coldiff && rowdiff != -coldiff && -rowdiff != coldiff) return FAILURE;
...
{
    if (!image || !extension) return FAILURE;

    int oldstrlen = strlen(image->name);
    int maxstrlen = oldstrlen + strlen(extension) + 1;
    char *newname = malloc(maxstrlen);
    if (!newname) return FAILURE;
    ...
    if (lastdotfound == -1)
    {
        ...
        for (int i = oldstrlen; i < maxstrlen; i++)
        {
            newname[i] = '\0';
        }
    }
    ...
    {
        for (int i = 0; i < lastdotfound; i++)
        {
            newname[i] = image->name[i];
        }
        for (int i = lastdotfound; i < maxstrlen - 1; i++)
        {
            newname[i] = extension[i - lastdotfound];
        }
        newname[maxstrlen - 1] = '\0';
    }
    free(image->name);
    image->name = newname;
    return SUCCESS;
