...
struct my_stack *create_stack(void)
{
    ...
    stack->numElements = 0;
    stack->capacity = 1;
    if ((stack->elements = malloc(sizeof(int))) == NULL)
    {
        free(stack);
        return NULL;
    }
...
{
    if (stack == NULL || stack->elements == NULL) return FAILURE;
    if (stack->numElements == stack->capacity)
    {
        if ((stack->elements = realloc(stack->elements,
            stack->capacity * 2 * sizeof(int))) == NULL) 
            {
                free(stack->elements);
                free(stack);
                return FAILURE;
...
{
    if (stack == NULL || stack->elements == NULL || dataOut == NULL) return FAILURE;
